...
def main():
    ...
    # Save Model
    joblib.dump(model, models_dir / 'baseline_rf.joblib')
    ...
    plt.title('Baseline Quant-Only Random Forest Confusion Matrix', fontsize=16)
    plt.ylabel('True Rating', fontsize=12)
    plt.xlabel('Predicted Rating', fontsize=12)
    plt.tight_layout()
    plt.savefig(out_dir / 'baseline_rf_confusion_matrix.png', dpi=300)
    plt.close()
    ...
    plt.title('Baseline KPI Feature Importance (Random Forest)', fontsize=14)
    plt.xlabel('Gini Importance', fontsize=12)
    plt.tight_layout()
    plt.savefig(out_dir / 'baseline_rf_feature_importance.png', dpi=300)
    plt.close()
