...
def main():
    ...
    for row in data_rows:
        ...
        # 2. Check if it's a rated company, and if so, check if tree exists
        if rating != 'NR' and source_indicator != '0':
            tree_folder_name = dest_json.stem  # e.g., 0001786248_2022_10-K_extracted
            tree_dir = final_trees_dir / tree_folder_name
    ...
    if len(missing_trees_jsons) == 0:
        logging.info("All rated companies have their RAPTOR trees successfully built! Nothing else to do.")
        return
        
    logging.info(f"Found {len(missing_trees_jsons)} rated companies missing their RAPTOR trees.")
    logging.info("Initializing RAPTOR Summarizer...")
    ...
    for idx, json_path in enumerate(missing_trees_jsons, 1):
        ...
        try:
            ...
            # Save the tree inside final_trees_directory / {stem}
            save_tree(tree, final_trees_dir / json_path.stem)
        except Exception as e:
            logging.error(f"Failed to build tree for {json_path.name}: {str(e)}")

    logging.info("Finished building missing trees!")
