...
def main():
    ...
    logging.info("Training Hybrid XGBoost model with optimal balanced capacity...")
    ...
    # Color code features: Quant (skyblue) vs YoY Delta (purple) vs Qual (lightgreen)
    colors = []
    for i in sorted_idx:
        feat = all_features[i]
    ...
    logging.info(f"Hybrid MAE (Notches):{mae:.4f}")
    logging.info(f"Hybrid Within-1-Notch: {within_1_notch:.2%}")
