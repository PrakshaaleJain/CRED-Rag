...
def main():
    ...
    sentiment_csv = project_root / 'data' / 'naive_sentiment_scores.csv'
    
    # Output path
    hybrid_out_csv = project_root / 'data' / 'naive_training_features.csv'
    ...
    logging.info("Loading naive qualitative sentiment scores...")
    ...
    # 5. Export
    logging.info(f"Exporting Naive Hybrid Dataset with {len(hybrid_df)} records and {len(kpi_features) + len(sent_features)} features...")
    hybrid_df.to_csv(hybrid_out_csv, index=False)
    
    logging.info(f"Success! Naive Hybrid dataset saved to {hybrid_out_csv}")
